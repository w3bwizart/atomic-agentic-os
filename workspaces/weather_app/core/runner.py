...
def execute_organism_agent_task(task_id: str, organism_agent_id: str, agent_config: dict, body: str, state_file: Path, sops: str, workspace_dir: str = "."):
    ...
    agent = OrganismAtomicAgent(
        config=AgentConfig(
            ...
            model_params={"max_tokens": 8192, "temperature": 0.5}
        )
    )
    ...
    try:
        ...
        if api_key in ["dummy-key", "ollama", "ENV_VAR"]:
            ...
            for attempt in range(max_retries):
                try:
                    # Enforce tool execution directly in the user prompt payload
                    payload_suffix = ""
                    if authorized_tools:
                        tool_schemas = "\\n".join([f"- tool_name: '{type(t).__name__}', schema: {json.dumps(t.input_schema.model_json_schema())}" for t in authorized_tools])
                        payload_suffix = f"\\n\\n[SYSTEM DIRECTIVE]: You MUST accomplish the user's objective by executing one of the available tool functions. To execute a tool, your entire `chat_message` response MUST be a single, valid JSON block matching this exact format: {{\"tool_name\": \"[Selected Tool]\", \"parameters\": {{...}} }}. Do NOT wrap the JSON in markdown code blocks. Do NOT output conversational text.\\n\\nAvailable Tools:\\n{tool_schemas}"
                    
                    response = agent.run(agent.input_schema(chat_message=body + payload_suffix))
                    ...
                    if authorized_tools:
                        ...
                        try:
                            ...
                            logger.info(f"RAW EXTRACTED TEXT BEFORE JSON LOADS: {raw_text}")
                            parsed_json = None
                            try:
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            try:
                                                ...
                                                logger.warning(f"OS Atomic Execution: Chat response is not valid JSON. Ignoring tools. Raw: {raw_text}. Error: {e}")
                                                parsed_json = None
                            
                            executed = False
                            if parsed_json:
                                target_tool_name = parsed_json.get("tool_name")
                                target_params = parsed_json.get("parameters", parsed_json)
                                
                                for tool in authorized_tools:
                                    if target_tool_name and tool.__class__.__name__ != target_tool_name:
                                        continue
                                    try:
                                        validated_input = tool.input_schema(**target_params)
                                        logger.info(f"OS Atomic Execution: Natively triggering {tool.__class__.__name__} via parsed JSON hook.")
                                        tool_result = tool.run(validated_input)
                                        final_response = getattr(tool_result, 'message', str(tool_result))
                                        executed = True
                                        break
                                    except Exception as e:
                                        continue
                            ...
                            if not executed:
                                logger.warning("OS Atomic Execution: Valid JSON returned, but it failed to match any authorized tool schema.")
                        except json.JSONDecodeError:
                            pass
                    ...
                    if not final_response or final_response.strip() == "":
                        ...
                        try:
                            ...
                            if receiver:
                                ...
                                review_dir = Path(workspace_dir) / ".organism_agents" / "review"
