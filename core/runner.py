            ...
            if getattr(client, "api_key", "") == "dummy-key":
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        logger.warning(f"LLM Error on attempt {attempt + 1}/{max_retries} for {agent_id}: {llm_e}")
                        if attempt < max_retries - 1:
                            import time
                            time.sleep(retry_delay)
                            retry_delay *= 2 # Exponential backoff
                        else:
                            logger.error(f"LLM exhausted all {max_retries} retries for {agent_id}. Final Error: {llm_e}")
                            final_response = f"Error during execution after retries: {llm_e}"
                            state_data["current_step"] = "failed"
        except Exception as e:
            logger.error(f"Global LLM Runner Error: {e}")
            final_response = f"Error during execution tracking: {e}"
            state_data["current_step"] = "failed"
